...
import qualified MultiIntersectV4OnePass as OP
import qualified MultiIntersectV5RHS     as RHS
import qualified MultiIntersectV6Stack   as STACK
...
prop_set_onepass_eq :: [Int] -> [Int] -> [Int] -> [Int] -> Bool
prop_set_onepass_eq raw1 raw2 raw3 raw4 =
  eq (mkHHSet raw1) (mkHHSet raw2) (mkHHSet raw3) (mkHHSet raw4)
     (S.fromList raw1) (S.fromList raw2) (S.fromList raw3) (S.fromList raw4)
  where
    eq hh1 hh2 hh3 hh4 s1 s2 s3 s4 =
      let nu = unionall $ (OP.onePassIntersection [hh1, hh2, hh3, hh4])
          si = S.intersection s4 $ S.intersection s3 $ S.intersection s2 s1
      in nu == si

    -- The output of nuIntersect is a list of non-overlapping sets. To compare
    -- to S.intersection, we must union them together.
    unionall :: Ord a => [ArraySet a] -> Set a
    unionall [] = S.empty
    unionall xs = S.fromList $ ssetToAscList $ foldl1 ssetUnion xs
...
prop_set_rhs_eq :: [Int] -> [Int] -> [Int] -> [Int] -> Bool
prop_set_rhs_eq raw1 raw2 raw3 raw4 =
  eq (mkHHSet raw1) (mkHHSet raw2) (mkHHSet raw3) (mkHHSet raw4)
     (S.fromList raw1) (S.fromList raw2) (S.fromList raw3) (S.fromList raw4)
  where
    eq hh1 hh2 hh3 hh4 s1 s2 s3 s4 =
      let nu = unionall $ (RHS.rhsIntersection [hh1, hh2, hh3, hh4])
          si = S.intersection s4 $ S.intersection s3 $ S.intersection s2 s1
      in nu == si
...
prop_set_stack_eq :: [Int] -> [Int] -> [Int] -> [Int] -> Bool
prop_set_stack_eq raw1 raw2 raw3 raw4 =
  eq (mkHHSet raw1) (mkHHSet raw2) (mkHHSet raw3) (mkHHSet raw4)
     (S.fromList raw1) (S.fromList raw2) (S.fromList raw3) (S.fromList raw4)
  where
    eq hh1 hh2 hh3 hh4 s1 s2 s3 s4 =
      let nu = unionall $ (STACK.stackIntersection [hh1, hh2, hh3, hh4])
          si = S.intersection s4 $ S.intersection s3 $ S.intersection s2 s1
      in nu == si
...
tests =
  testGroup "HitchhikerSet" [
    ...
    testProperty "Multi one pass v4" prop_set_onepass_eq,
    testProperty "Multi rhs v5" prop_set_rhs_eq,
    testProperty "Multi stack v6" prop_set_stack_eq
    ]
